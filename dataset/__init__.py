...
def create_dataset(dataset, config, corruption=None, severity=None, no_jsd=False):
    ...
    if dataset=='pretrain':
    ...
    elif dataset=='re_IC': # for evaluation
        train_dataset = re_train_dataset(config['train_file'], train_transform, config['image_root'])
        val_dataset = re_eval_dataset(config['val_file'], test_transform, config['image_root'])  
        test_dataset = re_eval_dataset_IC(config['test_file'], test_transform, config['image_root'], corruption=corruption, severity=severity)                
        return train_dataset, val_dataset, test_dataset   
        ...
        train_dataset = ve_dataset(config['train_file'], train_transform, config['image_root'])  
        val_dataset = ve_dataset(config['val_file'], test_transform, config['image_root'])  
        test_dataset = ve_dataset_IC(config['test_file'], test_transform, config['image_root'], corruption=corruption, severity=severity)                
        return train_dataset, val_dataset, test_dataset 
    
    elif dataset=='ve_aug':          
        train_dataset = ve_dataset_aug(config['train_file'], train_transform, config['image_root'])  
        val_dataset = ve_dataset(config['val_file'], test_transform, config['image_root'])  
        test_dataset = ve_dataset(config['test_file'], test_transform, config['image_root'])                
        return train_dataset, val_dataset, test_dataset  
