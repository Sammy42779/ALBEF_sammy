#!/bin/bash
#SBATCH -o ./fine_tune_weight_label_assign/job.%j.out    # 脚本执行的输出将被保存在当job.%j.out文件下，%j表示作业号;
#SBATCH -p v100          # 作业提交的指定分区队列为v100;如果使用rtx，则为-p titan
#SBATCH --qos=v100       # 指定作业的QOS为v100; 如果使用titan，则为--qos=titan
#SBATCH -J VE_weight_label_assign_jq        # 作业在调度系统中的作业名为myFirstJob;
...
--output_dir ${path_dir}/multimodal_models/ALBEF_sammy/fine_tune_weight_label_assign/VE \
