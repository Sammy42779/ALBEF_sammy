...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i,(images, text, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        ## gair weight
        loss = model(images, text_inputs, targets=targets, train=True, alpha=alpha, weight=args.weight)    
...
def evaluate(model, data_loader, tokenizer, device, config):
    ...
    for images, text, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        text_inputs = tokenizer(text, padding='longest', return_tensors="pt").to(device)   # shape=[bs, 14]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default='/ssd2/ld/ICCV2023/multimodal_models/ALBEF_sammy/fine_tune/VE/checkpoint_best.pth')   
    ...
    parser.add_argument('--weight', default='kl')
